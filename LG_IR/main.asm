...
; LG_IR.asm
;
; Created: 16/9/2026 20:25:35
; Author : Jorge
;
;
; Programar CKSEL[1:0] = 010 /LFuse = 0x7A (RC interno a 9.6Mhz)
;
...
RESET:
	...
	ldi r16, 126 ; cuenta hasta 127-1, unos 13uSeg
	out OCR0A,R16
...
; Configurar bits SM1=1, SM0=0 (Power-down) y SE=1 (Sleep Enable)
    ...
    sleep               ; El ATtiny13 se duerme por completo aquí
...
delay_560us:
    ldi r25, 7              ; Multiplicador externo (1 ciclo)
X1: ldi r24, 255            ; Multiplicador interno (1 ciclo) 248/250
X2: dec r24                 ; 1 ciclo
...
delay_4500us:
    ldi r25, 57             ; Multiplicador externo (1 ciclo)
Y1: ldi r24, 255            ; Multiplicador interno (1 ciclo)
Y2: dec r24                 ; 1 ciclo
...
delay_100ms:
    ldi r25, 5
Z0:	push r25
    ldi r25, 253            ; Multiplicador externo (1 ciclo)
